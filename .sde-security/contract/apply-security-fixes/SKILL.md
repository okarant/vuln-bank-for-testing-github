...
| SDE note posting / note_count re-query | **script OK** | via `api_request` composite OR a script call, ONLY WITH completeness-verification (every `reference_id` reconciled) + retry (429/5xx/timeout/partial) |
...
- [ ] Each ML_DOC/external INFRA is documented in skill file (PROCESS excluded -- already noted by configure skill)
...
- [ ] **`.sde-handoff.json` (setup-security-plan's) was NOT modified or deleted -- Step 6.5 invariant**
...
- PROCESS (note-only, handled by configure skill): {process_count}
...
- Configure-survey handoff: .sde-handoff.json preserved
...
>
> **Sequencing note:** Step 0 runs BEFORE handoff detection (Step 0.5), so you will not yet know `source_skill` when you build the master vuln list. That is fine — a greenfield scaffold naturally yields an empty/near-empty Step 0 list. Do NOT force-fill it; once Step 0.5 confirms `source_skill == "create-security-plan-from-specs"`, reconcile by treating that empty list as expected (not a failure) and applying the greenfield rules above.
>
> Brownfield mode (upstream = `setup-security-plan-from-repo`, or no handoff) keeps the full vulnerability-centric behavior below.
...
**If handoff file found AND `source_skill` is one of `"setup-security-plan-from-repo"` or `"create-security-plan-from-specs"`:**

1. Load all values from handoff:
   - `repository_path`
   - `project_id`, `project_name`
   - `business_unit_id`, `application_id`
   - `risk_policy_id` (may be null if skipped at creation time)
   - `agents_md`, `skill_files` (now per-CM paths: `skills/{domain}/{cm-slug}/SKILL.md`)
...
- **If missing**: STOP - run `@sde-skills/setup-security-plan-from-repo` first (recommended) or `@sde-skills/create-security-specs-from-sde-countermeasures` if survey already exists
...
- **If missing or empty**: STOP - run `@sde-skills/setup-security-plan-from-repo` first (recommended) or `@sde-skills/create-security-specs-from-sde-countermeasures` if survey already exists
...
**The AGENTS.md ledger is the authoritative per-CM status/scope source.** Derive the CM set AND each CM's status and category from the **AGENTS.md ledger rows** (the Countermeasure Index / Progress Tracking table), which covers ALL file-tracked CMs including Format B. Do NOT derive status/category by parsing a per-file `**Status:**` / `**Category:**` field: Format B (library-sourced, byte-exact) SKILL.md files have NO such field, so per-file status parsing silently drops them. Each ledger row maps to exactly one `skills/**/SKILL.md` file on disk; if a CM's file exists on disk but is missing from the ledger, add its row (reconciliation, Step 2.6).
...
1. Enumerate the AGENTS.md ledger rows (the per-CM index table) — these are authoritative and cover ALL file-tracked CMs (Format A, B, and C). Each row's Status column is that CM's status.
...
```
   ...
   Then emit the cumulative RUNNING CHECK (denominator = scoped_total; artifact = terminal-status SKILL.md files, counted via shell, NOT in-context):
```
[RUNNING CHECK] terminal-status SKILL.md files {disk_count} | expected (scoped_total) {scoped_total} | batches {B}/{total} | on track? {YES/NO} | reloaded this step from disk? {YES} | sentinel: "{verbatim line from this step's section in .sde-security/contract/<skill>/SKILL.md}"
```
...
**PROCESS countermeasures are excluded** -- they have no skill files (already noted in SD Elements by the configure skill). Only CODE_FIX, ML_CODE, ML_DOC, and INFRA countermeasures appear in the processing loop.

**MANDATORY PATTERN:**

```python
# Filter CMs by selected scope, EXCLUDING already-done CMs.
# PROCESS CMs have no skill files -- they were noted in SD Elements by the configure skill.
#
# AUTHORITATIVE SOURCE = the AGENTS.md ledger rows. Derive the CM set AND each CM's
# status + category FROM THE LEDGER ROWS (the per-CM index table), which cover ALL
...
all_cms = parse_agents_md_ledger_rows()   # one row per file-tracked CM: id, domain, slug/skill_file, category, status
pending_cms = [cm for cm in all_cms if cm.status == "Pending"]   # status read FROM the ledger row
# (Per-CM recipe details -- title, target files, fix text -- are still read from each CM's
#  SKILL.md inside the loop below, as required for the CM PROCESSING PROOF block.)
...
for cm in scoped_cms:
    ...
    if scope == "one_by_one":
        ...
        if response == "skip":
            update_cm_status(cm, "Skipped")  # in SKILL.md and AGENTS.md index
            skipped += 1
            done += 1
    ...
    # The AGENTS.md LEDGER ROW is the authoritative per-CM status. Update it for EVERY CM
    # (Pending -> Applied/Documented/Skipped). This is a single-row ledger edit (allowed;
    # NOT a batch shell update, so Rule 8 stays intact).
    update_agents_md_index_row(cm, new_status)            # MANDATORY for every CM (authoritative)
    ...
    # Accumulate the audit comment for a BATCHED composite POST (do NOT post inline).
    # The note_text is built now (while the local SKILL.md still exists -- Step 7 may
    # delete it later), but the actual POST happens once per batch via the Composite API.
...
===
""")
    print(f"[PROGRESS] {done}/{scoped_total} | {cm.id}: {new_status} | Note: PENDING (flushed at batch end) | Remaining: {scoped_total - done}")

    # === BATCH NOTE FLUSH (Composite API) ===
    # At each batch boundary (every 20 CMs) AND once after the final partial batch,
    # post ALL accumulated notes for the batch with ONE composite call:
    #
    #   api_request(method="POST", endpoint="composite/", data={
    #     "all_or_none": False,
    ...
    #   })
    #
    # Parse composite_response by reference_id:
    #   - http_status_code 201 -> notes_added += 1; set that CM's audit_records.sde_note_result = "ADDED"
    #   - 4xx/5xx -> append {id, full_id, note_text} to notes_failed[]; sde_note_result = "FAILED"
    # Then clear pending_notes = []. Emit a [PROGRESS] note-status update per CM from the response.

# Retry failed notes — re-POST the original cached note_text via a single follow-up composite call.
# Do NOT re-read the local SKILL.md here: by retry time, Step 7 may already have deleted it.
if notes_failed:
    print(f"[RETRY] Retrying {len(notes_failed)} failed note(s) via ONE composite call...")
    # Build ONE composite POST containing only the failed CMs (same notes/ path + body as the flush above).
    # Parse the response; for each 201 do notes_added += 1; any entry still non-201 stays in still_failed[].
    still_failed = [f for f in notes_failed if composite_retry_status(f) != 201]
    notes_failed = still_failed
    if notes_failed:
...
> **SKILL.md Format Detection (MANDATORY):** Task recipe files come in three formats depending on which upstream skill generated them. Detect the format BEFORE locating vulnerable code:
>
> **Format A — Template-generated by `setup-security-plan-from-repo`:** Uses "Code to Fix" (code snippets with file:line references) and "Required Fix" (secure code pattern) sections. Locate vulnerable code using the "Code to Fix" section; apply the pattern from "Required Fix".
>
...
- Track failed notes in a `notes_failed` list.
- After the main processing loop completes, retry all failed notes once. **The retry payload MUST be the same fully-embedded note that was prepared inline** (do NOT regenerate by reading the local file — by the time of retry, you may already be near Step 7 or executing it; cache the prepared `note_text` in `notes_failed[]` alongside the CM id and `full_id`).
...
> **Greenfield (create-security-plan upstream): N/A.** If this run is greenfield (see Step 0.0 GREENFIELD MODE — `source_skill == "create-security-plan-from-specs"` or Format C), there are no pre-existing vulnerable surfaces to keep in parity. Emit `Additional surfaces found: 0 (greenfield)` and skip to the next step.
...
Re-derive every CM's terminal status from the **AGENTS.md ledger rows** (the authoritative per-CM status/scope source) and verify every SCOPED CM has a terminal status. This catches CMs that were silently dropped. **Count terminal status from the LEDGER rows; the completeness assertion is `M_scoped == scoped_total`** (the CMs in scope for THIS run — for `scope=all`, `scoped_total == N_ledger == file_tracked`) — NOT from per-file `**Status:**`. Format B (library-sourced, byte-exact) files have NO in-file `**Status:**`, so a statusless Format B file is NOT a failure; its status is its ledger row.
...
SKILL.md files on disk: {N} (Format A: {a}, Format B/library: {b}, Format C: {c})
Ledger rows (AGENTS.md per-CM index): {N_ledger}
SCOPED CMs with a terminal status (Applied/Documented/Skipped) IN THE LEDGER ROW: {M_scoped} (scoped_total: {scoped_total})
VERIFY: M_scoped == scoped_total (every SCOPED CM has a terminal ledger status, none dropped; for scope=all, scoped_total == N_ledger == file_tracked)? {YES/NO}
VERIFY: N_ledger == N (one ledger row per SKILL.md file on disk)? {YES/NO}
...
1. Write the script to `{repository_path}/.sde-security/verify-apply-output.sh`. It is **DISK-ONLY and CONTEXT-LIGHT** (counts via shell/grep; NEVER reads file bodies into context). It MUST:
   - Count **AGENTS.md ledger rows** (the authoritative per-CM index) that carry a terminal status (`Applied`, `Documented`, or `Skipped`) and assert the count equals `scoped_total`. The ledger — NOT per-file `**Status:**` — is the source of truth (Format B files have no in-file status; a statusless Format B file is NOT a failure)
   - For each scoped CM ID, assert its **ledger row** has a terminal status; list any missing
   - Assert `grep -c '^| T[0-9]' AGENTS.md` (ledger rows) equals the count of `skills/**/SKILL.md` files on disk (one ledger row per file) and is consistent with the scoped/total CM set (tri-source: SDE scoped_total == ledger rows == SKILL.md files on disk)
   ...
   - Exit non-zero (`VERIFY FAIL: terminal-status shortfall`) if any scoped CM lacks a terminal LEDGER status
...
B. Disk (actual), via shell/grep ONLY: terminal-status LEDGER row count (AGENTS.md, authoritative);
   grep -c ledger rows; SKILL.md file count on disk.
C. INVARIANT: scoped_total == terminal-status LEDGER rows == SKILL.md files on disk for scoped CMs? {YES/NO}
...
The setup-security-plan skill writes `.sde-handoff.json` at the repository root. **This skill MUST NOT touch that file.** The apply-fixes handoff goes to a **distinct filename**: `.sde-apply-handoff.json`.

| File | Owner | This skill may |
|------|-------|----------------|
| `.sde-handoff.json` | `setup-security-plan-from-repo` | **READ ONLY** (in Step 0.5). NEVER write, rename, or delete. |
...
**Intentionally NOT persisted here** (re-fetchable from SDE by code-scan-verification-validation via `project_countermeasures op=get expand=text,problem,tags,how_tos,name`): `title`, `domain`, `fix_summary`, `sde_note_text`, `project_name`, `risk_policy_id`.
...
assert handoff_path.endswith(".sde-apply-handoff.json"), \
    "NEVER write to .sde-handoff.json -- that belongs to setup-security-plan"
...
assert os.path.exists(f"{repository_path}/.sde-handoff.json"), \
    ".sde-handoff.json must still exist (setup-security-plan's handoff)"
...
[CHECKPOINT] Configure-survey handoff preserved: .sde-handoff.json (untouched)
```
...
**Note:** AGENTS.md was NOT archived by the setup-security-plan skill (it was merged instead). Do NOT attempt to restore AGENTS.md from the archive.

**Purpose:** Restore pre-existing AI assistant configuration files that were archived by the setup-security-plan skill.
...
if [ -f ".sde-ai-backup.tar.gz" ]; then
    # Archive exists - restore files
    tar -xzvf .sde-ai-backup.tar.gz
    rm .sde-ai-backup.tar.gz
    echo "AI config files restored"
else
    echo "No AI config archive found"
fi
```
...
- All ML_DOC/INFRA countermeasures documented (PROCESS already noted by configure skill)
...
- upstream_handoff: .sde-handoff.json (setup-security-plan, preserved)
